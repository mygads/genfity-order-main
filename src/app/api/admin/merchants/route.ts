...
async function getMerchantsHandler(request: NextRequest) {
  ...
  const merchants = await Promise.all(merchantsData.map(async (merchant: Record<string, unknown>) => {
    ...
    return {
      ...
      parentMerchantName: (merchant.parentMerchant as Record<string, unknown> | undefined)?.name || null,
      branchesCount: Array.isArray(merchant.branches) ? merchant.branches.length : 0,
