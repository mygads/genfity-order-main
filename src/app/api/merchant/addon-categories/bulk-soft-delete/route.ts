/**
 * Bulk Delete Addon Categories API
 * POST /api/merchant/addon-categories/bulk-soft-delete - Soft delete multiple addon categories at once
 *
 * @description Performs soft delete on multiple addon categories.
...
function generateConfirmationToken(ids: number[]): string {
  ...
  return `DELETE_ADDON_CATS_${sortedIds.length}_ITEMS_${sortedIds[0]}_${sortedIds[sortedIds.length - 1]}`;
}
  ...
  try {
    ...
    if (!ids || !Array.isArray(ids) || ids.length === 0) {
      return NextResponse.json(
        { success: false, error: 'INVALID_IDS', message: 'Please provide an array of addon category IDs' },
        { status: 400 }
      );
    }
    ...
    if (ids.length > 50) {
      return NextResponse.json(
        { success: false, error: 'TOO_MANY_ITEMS', message: 'Cannot delete more than 50 addon categories at once' },
        { status: 400 }
      );
    }
    ...
    // Perform bulk delete
    const result = await dataCleanupService.bulkDeleteAddonCategories(
      ids.map(id => BigInt(id)),
      BigInt(userId)
    );

    console.log(`✅ Bulk deleted ${result.deletedCount} addon categories by user ${userId}`);

    return NextResponse.json({
      success: true,
      message: `Successfully deleted ${result.deletedCount} addon categories`,
      data: serializeBigInt(result),
    });
  } catch (error) {
    console.error('❌ Error bulk deleting addon categories:', error);

    return NextResponse.json(
      {
        success: false,
        error: 'BULK_DELETE_FAILED',
        message: error instanceof Error ? error.message : 'Failed to bulk delete addon categories',
      },
      { status: 500 }
    );
  }
  ...
  try {
    ...
    return NextResponse.json({
      ...
      data: {
        itemCount: ids.length,
        confirmationToken,
        message: `This will delete ${ids.length} addon categories. Use this token to confirm.`,
      },
    });
  } catch (error) {
