...
import { CardSkeleton, Skeleton } from '@/components/common/SkeletonLoaders';
...
function SearchPageContent() {
  ...
  const fallbackRefUrl = customerOrderUrl(merchantCode, { mode });
  const { pushBack } = useCustomerBackTarget({ fallback: fallbackRefUrl });
  ...
  const removeActiveChip = (chipKey: string) => {
    if (chipKey.startsWith('cat:')) {
      const id = chipKey.slice('cat:'.length);
      setSelectedCategories((prev) => prev.filter((x) => x !== id));
      return;
    }
    if (chipKey.startsWith('tag:')) {
      const key = chipKey.slice('tag:'.length);
      setSelectedDietaryFilters((prev) => prev.filter((x) => x !== key));
      return;
    }
    if (chipKey === 'price') {
  ...
  const activeFilterChips = useMemo(() => {
    ...
    const categoryNameById = new Map(availableCategories.map((c) => [c.id, c.name]));
    for (const id of selectedCategories) {
      const name = categoryNameById.get(id);
      if (name) chips.push({ key: `cat:${id}`, label: name });
    }

    const dietaryByKey = new Map(DIETARY_FILTER_KEYS.map((f) => [f.key, f] as const));
    for (const key of selectedDietaryFilters) {
      const f = dietaryByKey.get(key);
      if (f) chips.push({ key: `tag:${key}`, label: t(f.labelKey) });
    }

    if (priceRange[0] > 0 || priceRange[1] < maxPrice) {
      chips.push({
        key: 'price',
        label: `${formatCurrency(priceRange[0], merchantInfo?.currency || 'AUD')} - ${formatCurrency(priceRange[1], merchantInfo?.currency || 'AUD')}`,
      });
    }

    return chips;
  }, [availableCategories, selectedCategories, selectedDietaryFilters, priceRange, maxPrice, merchantInfo?.currency, t]);
  ...
  return (
    <div className="min-h-screen bg-white">
                ...
                <div className="inline-flex items-center gap-2">
                  {activeFilterChips.slice(0, 8).map((chip) => (
                    <button
                      ...
                      aria-label={`${chip.label} (${t('common.remove') || 'Remove'})`}
                    >
                      <span className="max-w-45 truncate">{chip.label}</span>
                      <X className="ml-2 w-3.5 h-3.5 text-gray-400" />
                    </button>
                  ))}
      ...
      {/* Filter Panel */}
      {showFilters && (
        <div className="border-b border-gray-200 bg-gray-50 px-4 py-4 space-y-4">
          {/* Sort Dropdown */}
          <div>
