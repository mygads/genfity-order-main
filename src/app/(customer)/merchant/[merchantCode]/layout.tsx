...
import { setLastMerchantCodeClient } from '@/lib/utils/merchantContext';
...
function MerchantLayoutInner({ children }: MerchantLayoutProps) {
  ...
  // Remember last merchant context (used for legacy /profile, /history redirects)
  useEffect(() => {
    const code = merchantCode?.trim();
    if (!code) return;
    setLastMerchantCodeClient(code);
  }, [merchantCode]);
