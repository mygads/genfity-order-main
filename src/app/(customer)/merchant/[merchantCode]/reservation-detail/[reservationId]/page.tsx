...
import { customerHistoryUrl, customerLoginUrl, customerOrderDetailUrl } from '@/lib/utils/customerRoutes';
...
function ReservationDetailPageContent() {
  ...
  useEffect(() => {
    ...
    const fetchReservation = async () => {
      ...
      if (!auth) {
        router.push(customerLoginUrl({ merchant: merchantCode, mode, ref: window.location.pathname + window.location.search }));
        return;
      }
        ...
        {reservation.order?.orderNumber ? (
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-200">
            ...
            <button
              onClick={() => {
                router.push(
                  customerOrderDetailUrl(merchantCode, reservation.order!.orderNumber, {
...
function ReservationDetailFallback() {
  return (
...
export default function ReservationDetailPage() {
  return (
    <Suspense fallback={<ReservationDetailFallback />}>
