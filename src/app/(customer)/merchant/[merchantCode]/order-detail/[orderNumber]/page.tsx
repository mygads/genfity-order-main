...
import { customerHistoryUrl, customerLoginUrl } from '@/lib/utils/customerRoutes';
...
function OrderDetailPageContent() {
    ...
    const backTarget = useCustomerBackTarget({
        merchantCode,
        mode,
        fallback: customerHistoryUrl(merchantCode, { mode }),
    });
    ...
    useEffect(() => {
        const fetchOrder = async () => {
            ...
            if (!auth) {
                ...
                router.push(customerLoginUrl({ merchant: merchantCode, mode, ref: currentPath }));
                return;
            }
                        ...
                        <div className="flex flex-col items-end gap-2">
                            ...
                            {order.changedByAdmin ? (
                                <span className="px-2 py-0.5 rounded-full bg-gray-100 text-gray-600 text-[10px] font-semibold">
                                    {tOr(t, 'customer.orderDetail.changedByAdmin', 'Changed by admin')}
                                </span>
                            ) : null}
                        </div>
                    </div>
