...
import {
  ...
  customerOrderSummaryOnlineUrl,
  customerOrderUrl,
  customerViewOrderUrl,
...
function PaymentPageContent() {
    ...
    if (cart !== null && (!cart || cart.items.length === 0)) {
      ...
      router.push(
        customerOrderUrl(merchantCode, {
          ...
          ...(isScheduledRequested ? { scheduled: 1 } : {}),
        })
      );
    }
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      ...
      const orderNumber = String(orderData.data.orderNumber);
      router.push(
        isOnlinePayment
          ? customerOrderSummaryOnlineUrl(merchantCode, {
              orderNumber,
              mode,
              ...(trackingToken ? { token: trackingToken } : {}),
            })
          : customerOrderSummaryCashUrl(merchantCode, {
              orderNumber,
              mode,
              ...(trackingToken ? { token: trackingToken } : {}),
            })
