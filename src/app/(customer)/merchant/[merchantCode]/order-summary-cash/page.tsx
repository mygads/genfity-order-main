...
import { customerHistoryUrl, customerMerchantHomeUrl, customerOrderUrl, customerTrackUrl } from '@/lib/utils/customerRoutes';
 ...
 */
function OrderSummaryCashPageContent() {
  ...
  const { safeRef: safeCloseRef } = useCustomerBackTarget({ fallback: customerMerchantHomeUrl(merchantCode) });

  const trackBack = openedFrom === 'history' ? 'history' : undefined;
  const trackRef = openedFrom === 'history' && safeCloseRef ? safeCloseRef : undefined;
  ...
  const handleClose = () => {
    if (openedFrom === 'history' && safeCloseRef) {
      router.push(safeCloseRef);
      return;
    }
  ...
  const _handleViewHistory = () => {
    router.push(customerHistoryUrl(merchantCode, { mode }));
  };
...
export default function OrderSummaryCashPage() {
  return (
    <Suspense fallback={<OrderSummaryCashSkeleton />}>
      <OrderSummaryCashPageContent />
    </Suspense>
  );
}
