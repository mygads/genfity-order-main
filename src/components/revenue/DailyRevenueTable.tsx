...
export default function DailyRevenueTable({
  ...
  // Check if any row has service charge or packaging fee
  const hasServiceCharge = data.some(item => (item.totalServiceCharge || 0) > 0);
  const hasPackagingFee = data.some(item => (item.totalPackagingFee || 0) > 0);
  ...
  const totals = data.reduce(
    (acc, item) => ({
      ...
      totalServiceCharge: acc.totalServiceCharge + (item.totalServiceCharge || 0),
      totalPackagingFee: acc.totalPackagingFee + (item.totalPackagingFee || 0),
      grandTotal: acc.grandTotal + item.grandTotal,
    }),
    { totalOrders: 0, totalRevenue: 0, totalTax: 0, totalServiceCharge: 0, totalPackagingFee: 0, grandTotal: 0 }
  );

  const colSpan = 5 + (hasServiceCharge ? 1 : 0) + (hasPackagingFee ? 1 : 0);
